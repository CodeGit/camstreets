...
-- look at. 2026-09-07 is a Monday, so day_of_week = 1 picks the matching
-- row out of the 5 weekday rows each label now has.
insert into public.slot_instances (slot_id, term_id, date, start_time, end_time, capacity, status) values
  (
    (select id from public.slots where label = 'Morning drop-off' and day_of_week = 1),
    (select id from public.terms where name = 'Autumn 1 2026'),
    '2026-09-07', '08:15', '08:45', 2, 'open'
  ),
  (
    (select id from public.slots where label = 'Afternoon pickup' and day_of_week = 1),
    (select id from public.terms where name = 'Autumn 1 2026'),
