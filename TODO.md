...
- [x] Public demo mode for `dev.camstreets.org`: 3 pre-seeded schools (scripts/seed-demo.mjs) and a self-service "become an admin" button (becomeDemoAdmin, gated by ALLOW_SELF_ELEVATE + schools.is_demo - see the plan at the time, sequential-nibbling-journal.md, for the full design), verified locally end-to-end
  - [ ] Still needed before this is live on the real dev.camstreets.org: add `SUPABASE_SERVICE_ROLE_KEY` and `ALLOW_SELF_ELEVATE=true` to Vercel's Preview environment scope (the former is also needed for the calendar-feed route above, which doesn't work on dev.camstreets.org yet either); run `pnpm run db:seed-demo` once against the real dev project; add `DEMO_SUPABASE_URL`/`DEMO_SUPABASE_SERVICE_ROLE_KEY` as GitHub Actions repo secrets so `.github/workflows/reset-demo.yml`'s nightly reset can run
